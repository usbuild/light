...
#define HANDLER_LOG(x) DLOG(x) << name << "[" <<  static_cast<int>(station_id_) << "] "

		using nsm_t = light::service::NetworkServiceMessage;
		using nsmt_t = light::service::NetworkServiceMessageType;

		Station::Station(uint8_t station_id, const light::network::INetEndPoint& endpoint,
			network_service_ptr_t network_service) : station_id_(station_id), endpoint_(endpoint), network_service_(network_service) {}

		Station::~Station() {}

		light::utils::ErrorCode  Station::init() {
			return LS_OK_ERROR();
		}

		void Station::on_install() {
			using ns_t = light::service::NetworkService;
			network_service_->post<ns_t>(&ns_t::create_tcp_server, endpoint_, 5, ctx_->get_looper().wrap([this](light::utils::ErrorCode ec, uint32_t handle){
					UNUSED(handle);
					if (!ec.ok()) {
							HANDLER_LOG(INFO) << "unable to create tcp server, will exit or restart";
						} else {
							HANDLER_LOG(INFO) << "server listening on : " << endpoint_.to_string() << ":" << endpoint_.get_port();
						}
					}), id_);
		}
		...
		void Station::connect_to_station(const light::network::INetEndPoint &point) {
			using ns_t = light::service::NetworkService;
			network_service_->post<ns_t>(&ns_t::connect_tcp_server, point, 5000000LL, ctx_->get_looper().wrap([this, point](light::utils::ErrorCode ec, uint32_t handle){
					UNUSED(handle);
					if (ec.ok()) {
						HANDLER_LOG(INFO) << "connect: " << point.to_string() << ":" << point.get_port() << " " << ec.message();
					} else {
						HANDLER_LOG(INFO) << "error while connect: " << point.to_string() << ":" << point.get_port() << " " << ec.message();
					}
					}), id_);
		}
		...
		light::service::CommonPacket Station::generate_network_packet(light::core::mq_handler_id_t to, light::core::mq_handler_id_t from,
		...
		void Station::handle_socket_message(light::core::light_message_ptr_t msg) {
			...
			case nsmt_t::NET_MSG_TYPE_CONNECT:
				{
					HANDLER_LOG(INFO) << "got connection from: " << nsm->endpoint.to_string() << ":" << nsm->endpoint.get_port();
			...
			case nsmt_t::NET_MSG_TYPE_CLOSE:
				LOG(WARNING) << "connection closed " << nsm->endpoint.to_string() << ":" << nsm->endpoint.get_port();
				{
		...
		void Station::register_to_station(uint32_t target_handle, bool need_back) {
			...
			fbb.Finish(mloc);
			auto pkt = generate_network_packet(0, station_id_, StationMessageType::REGISTER, target_handle, fbb);
		...
		void Station::handle_inter_station_message(light::core::light_message_ptr_t msg, uint32_t handle) {
			if (msg->type == StationMessageType::REGISTER) {
				auto conn_request = flatbuffers::GetRoot<light::proto::ConnectRequest>(msg->data);
				...
				if (it == other_station_conn_.end() || !std::get<1>(it->second)) {
					...
					HANDLER_LOG(INFO) << "get register info from station_id=" << conn_request->my_id();
				} else {
					HANDLER_LOG(INFO) << "station with id=" << conn_request->my_id() << " already registered";
				}
			}
		}
