...
namespace light {
	namespace handler {
		...
		struct PartialPacketMessage {
			...
			inline bool header_ready() const {
				return data != nullptr;
			}

			inline size_t need_header_size() const {
				if (header_ready()) return 0;
		...
		class Station : public light::core::MessageHandler {
			...
			public:
			Station(uint8_t station_id, const light::network::INetEndPoint& endpoint, network_service_ptr_t network_service);
			~Station();
			...
			void handle_inter_station_message(light::core::light_message_ptr_t msg, uint32_t handle);

			light::service::CommonPacket generate_network_packet(light::core::mq_handler_id_t to, light::core::mq_handler_id_t from,
			...
			std::map<light::core::mq_handler_id_t, PartialPacketMessage> other_station_data_;
		};
