...
namespace light {
	namespace network {
		Looper::Looper():
				poller_(Poller::create_default_poller(*this)),
				stop_(false),
			...
			Dispatcher& Looper::get_time_dispatcher() {
				return *timer_dispatcher_;
			}

			Dispatcher& Looper::get_event_dispatcher() {
			...
			light::utils::ErrorCode Looper::update_timerfd_expire() {
				...
				if ((::timerfd_settime(timer_dispatcher_->get_fd(), TFD_TIMER_ABSTIME, &spec, nullptr)) == -1) {
				...
				while(true) {
					...
					do {
						...
						if (!unique_post_functors_.empty()) {
							for (auto it = unique_post_functors_.begin(); it != unique_post_functors_.end(); ++it) {
								auto unique_id = it->first;
								if (running_post_queue_ids_.find(unique_id) != running_post_queue_ids_.end()) continue;
		...
		int Looper::register_loop_callback(const loop_callback_t &func, int idx) {
			std::unique_lock<std::mutex> plk(post_functor_lock_);
			if (idx == -1) idx = ++last_callback_idx_;
