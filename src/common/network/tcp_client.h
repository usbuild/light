...
namespace light {
	namespace network {
		...
		class TcpClient : public TcpSocket, public std::enable_shared_from_this<TcpClient> {
			public:
				...
				template<typename ConnectHandler>
					void async_connect(const INetEndPoint& point, const ConnectHandler &on_connect, uint64_t timeout=76*1000000LL);
		...
		template<typename ConnectHandler>
			void TcpClient::async_connect(const INetEndPoint& point, const ConnectHandler &on_connect, uint64_t timeout) {
				auto ec = TcpSocket::connect(point);
				if (ec == LS_GENERIC_ERR_OBJ(in_progress) || ec.error_code() == ERRNO(EWOULDBLOCK)) {
