...
namespace light {
	namespace network {
		...
		TcpConnection::TcpConnection(Looper &looper): TcpSocket(), Connection(looper), write_buffer_(), bytes_has_read_(0) {}

		TcpConnection::TcpConnection(Looper &looper, int fd): TcpSocket(fd), Connection(looper), write_buffer_(), bytes_has_read_(0){
			dispatcher_.reset(new Dispatcher(looper, fd));
			auto ec = this->set_nonblocking();
			if (!ec.ok()) throw light::exception::SocketException(ec);
			dispatcher_->set_write_callback(std::bind(&TcpConnection::buffer_write_callback, this));
		}

		TcpConnection::~TcpConnection() {
			if (dispatcher_) dispatcher_->detach();
		}
		...
		void TcpConnection::async_write(void *buf, size_t len, const write_callback_t &func) {
		...
		light::utils::ErrorCode TcpConnection::get_peer_endpoint(light::network::INetEndPoint &endpoint) {
