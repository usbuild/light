	...
	namespace network {
		...
		class TcpConnection : public TcpSocket, public Connection {
			...
			public:
				...
				template<typename T>
					void set_error_callback(T &&t);

				template<typename T>
					void set_close_callback(T &&t);
			  ...
			  light::utils::ErrorCode get_peer_endpoint(light::network::INetEndPoint &endpoint);
		...
		template<typename T>
			void TcpConnection::set_error_callback(T &&t) {
				dispatcher_->set_error_callback(std::forward<T>(t));
			}
		template<typename T>
			void TcpConnection::set_close_callback(T &&t) {
				dispatcher_->set_close_callback(std::forward<T>(t));
			}

		template<typename ReadCallback>
			void TcpConnection::async_read(void *read_buf, size_t bytes_to_read, ReadCallback cb) {
				bytes_has_read_ = 0;
		...
		template<typename ReadCallback>
			void TcpConnection::async_read_some(void *read_buf, size_t buf_len, ReadCallback cb) {
				dispatcher_->enable_read();
