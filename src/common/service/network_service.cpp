...
#define GET_CONN_TYPE(key) (key >> CONN_TYPE_SHIFT)
		...
		static light::network::INetEndPointIpV4 ENetAddressToEndPoint(const ENetAddress &addr) {
		...
		static light::network::INetEndPoint get_tcp_peer_endpoint(light::network::TcpConnection *conn) {
		...
		NetworkService::NetworkService(light::core::Context &ctx):NetworkService(ctx.get_looper(), ctx.get_mq()) {
		}

		NetworkService::NetworkService(light::network::Looper &looper, light::core::MessageQueue &mq)
			: Service(looper, mq), last_socket_id_(0), last_callback_idx_(0), loop_idx_(0) {}
			...
			void NetworkService::forward_message(const NetworkServiceMessage &msg, uint32_t opaque) {
				...
				message->destroy = [](light::core::LightMessage &self){
					...
					if (p->packet.destroy) { p->packet.destroy(); }
					delete p;
				};
				get_mq().push_message(message);
			}

			void NetworkService::forward_data_message(NetworkServiceMessageType type, uint32_t opaque, uint32_t handle,
			...
			void NetworkService::forward_error_message(NetworkServiceMessageType type, uint32_t opaque, uint32_t handle,
				const light::utils::ErrorCode &ec, const light::network::INetEndPoint &peer) {
				NetworkServiceMessage msg;
				msg.type = type;
				msg.ec = ec;
				msg.endpoint = peer;
			...
			void NetworkService::forward_event_message(NetworkServiceMessageType type, uint32_t opaque, uint32_t handle,
			...
			uint32_t NetworkService::install_udp_connection(uint32_t host_handle, ENetPeer *peer, uint32_t opaque) {
			...
			void NetworkService::on_connect(uint32_t handle, ENetHost &host, const ENetEvent &event) {
				...
				if (it != connect_callbacks_.end()) {
					...
					forward_event_message(NetworkServiceMessageType::NET_MSG_TYPE_CONNECT, udp_host.opaque, key, ENetAddressToEndPoint(event.peer->address));
				}
			}

			void NetworkService::on_receive(uint32_t handle, ENetHost &host, const ENetEvent &event) {
				UNUSED(handle);
				...
				on_get_message_from_remote(handle, pkt, ENetAddressToEndPoint(event.peer->address));
			}

			void NetworkService::on_disconnect(uint32_t handle, ENetHost &host, const ENetEvent &event) {
				UNUSED(handle);
				uint32_t peer_handle = *static_cast<uint32_t*>(event.peer->data);
				delete static_cast<uint32_t*>(event.peer->data);
				DLOG(INFO) << "on_disconnect " << peer_handle << " " << &host << " " << event.peer;
				enet_peer_reset(event.peer);
				// dont call internal_close!!
				if (active_close_handlers_.find(peer_handle) != active_close_handlers_.end()) {
					...
					forward_event_message(NetworkServiceMessageType::NET_MSG_TYPE_CLOSE, conn.opaque, peer_handle, ENetAddressToEndPoint(event.peer->address));
			...
			void NetworkService::create_tcp_server(const light::network::INetEndPoint& endpoint, int backlog, network_service_callback_t func, uint32_t opaque) {/*{{{*/
				light::network::Acceptor* acceptor = new light::network::Acceptor(get_looper());
				...
				acceptor->set_accept_handler([this, opaque](const light::utils::ErrorCode &aec, int fd){
					if (aec.ok()) {
						uint32_t tcp_key;
						light::network::TcpConnection *conn;
						std::tie(tcp_key, conn) = install_tcp_connection(fd, opaque);
						forward_event_message(NetworkServiceMessageType::NET_MSG_TYPE_CONNECT, opaque, tcp_key, get_tcp_peer_endpoint(conn));
					} else {
			...
			void NetworkService::create_udp_server(const light::network::INetEndPoint& point, int max_peer, int max_channel,  network_service_callback_t func, uint32_t opaque) {/*{{{*/
			...
			void NetworkService::on_get_message_from_remote(uint32_t handle, const CommonPacket& pkt, const light::network::INetEndPoint& endpoint) {
				auto conn = connection_map_[handle];
				forward_data_message(NetworkServiceMessageType::NET_MSG_TYPE_DATA, conn.opaque, handle, pkt, endpoint);
			}


			void NetworkService::async_read_tcp_connection(light::network::TcpConnection *conn, uint32_t handle) {
				//use fixed allocator
				char *buf = fixed_alloc_.alloc();//2M
				conn->async_read_some(buf, fixed_alloc_.node_size(), [this, conn, handle, buf](light::utils::ErrorCode ec, size_t bytes_read){
					if (ec.ok()) {
						CommonPacket pkt;
						pkt.data = buf;
						pkt.size = bytes_read;
			...
			void NetworkService::handle_tcp_error(uint32_t handle, const light::utils::ErrorCode& ec) {
				auto conn = connection_map_[handle];
				forward_error_message(NetworkServiceMessageType::NET_MSG_TYPE_EXECPTION, conn.opaque, handle, ec, get_tcp_peer_endpoint(conn.tcp_conn));
				internal_close(handle, false);
			}

			void NetworkService::handle_tcp_close(uint32_t handle) {
				auto conn = connection_map_[handle];
				forward_event_message(NetworkServiceMessageType::NET_MSG_TYPE_CLOSE, conn.opaque, handle, get_tcp_peer_endpoint(conn.tcp_conn));
				internal_close(handle, false);
			}

			std::tuple<uint32_t, light::network::TcpConnection*> NetworkService::install_tcp_connection(int sockfd, uint32_t opaque) {
				uint32_t key = ++last_socket_id_;
				...
				conn->set_close_callback([conn, this, key](){
					handle_tcp_close(key);
					});
				return std::make_tuple(key, conn);
			}

			void NetworkService::connect_tcp_server(const light::network::INetEndPoint& point, uint64_t micro_sec, network_service_callback_t func, uint32_t opaque) {/*{{{*/
				light::network::TcpClient *tcp_client = new light::network::TcpClient(get_looper());
				auto tmp_ec = tcp_client->open(point.get_protocol());
				if (!tmp_ec.ok()) {
					func(tmp_ec, 0);
					return;
				}
				tcp_client->async_connect(point, [this, tcp_client, func, opaque](const light::utils::ErrorCode &ec){
					if (ec.ok()) {
						uint32_t key;
						light::network::TcpConnection *conn;
						std::tie(key, conn) = install_tcp_connection(tcp_client->get_sockfd(), opaque);
						func(LS_OK_ERROR(), key);
					} else {
						func(ec, 0);
					}
					//may be dangerous, but now ok, after all called, tcp_client is not touched
					delete tcp_client;
				}, micro_sec);
			}/*}}}*/

			void NetworkService::create_udp_stub(int max_peer, int max_channel,  network_service_callback_t func, uint32_t opaque) {/*{{{*/
				DLOG(INFO)<< "udp stub";
			...
			void NetworkService::connect_udp_server(const light::network::INetEndPoint& point, uint64_t micro_sec, int32_t stub_id, int channels, network_service_callback_t func, uint32_t opaque) {/*{{{*/
				...
				address.port = point.get_port();
				++last_callback_idx_;
				peer = enet_host_connect(host->second.udp_host, &address, channels, last_callback_idx_);
				if (peer == nullptr) {
			...
			void NetworkService::close(uint32_t handle) {
				internal_close(handle, true);
			}

			void NetworkService::internal_close(uint32_t handle, bool active_close) {/*{{{*/
				if (!check_handle_exists(handle)) return;
				auto conn = connection_map_[handle];
					...
					case CONN_TYPE_TCP_CLIENT:
						{
							...
							conn.tcp_conn->async_write(packet.data, packet.size, [packet]{ packet.destroy(); });
						}
						break;
					case CONN_TYPE_UDP_CLIENT:
						{
							//use packet pool?
							ENetPacket *enet_pkt = enet_packet_create(packet.data, packet.size,
																												(reliable ? ENET_PACKET_FLAG_RELIABLE : ENET_PACKET_FLAG_UNSEQUENCED) | ENET_PACKET_FLAG_NO_ALLOCATE);
