...
namespace light {
	namespace service
	{
		...
		public:
			typedef std::function<void(light::utils::ErrorCode, uint32_t)> network_service_callback_t;
		...
		public:
			...
			void create_tcp_server(const light::network::INetEndPoint& endpoint, int backlog, network_service_callback_t func, uint32_t opaque);

			void create_udp_server(const light::network::INetEndPoint& point, int max_peer, int max_channel,  network_service_callback_t func, uint32_t opaque);

			void connect_tcp_server(const light::network::INetEndPoint& point, uint64_t micro_sec, network_service_callback_t func, uint32_t opaque);

			void create_udp_stub(int max_peer, int max_channel,  network_service_callback_t func, uint32_t opaque);

			void connect_udp_server(const light::network::INetEndPoint& point, uint64_t micro_sec, int32_t stub_id, int channels, network_service_callback_t func, uint32_t opaque);
			 ...
			 */
			void send_common_packet(const CommonPacket &packet, bool reliable, int channel=0);
		...
		private:
			...
			void forward_data_message(NetworkServiceMessageType type, uint32_t opaque, uint32_t handle,
				const CommonPacket &packet, const light::network::INetEndPoint &peer);

			void forward_error_message(NetworkServiceMessageType type, uint32_t opaque, uint32_t handle,
				const light::utils::ErrorCode &ec, const light::network::INetEndPoint &peer);

			void forward_event_message(NetworkServiceMessageType type, uint32_t opaque, uint32_t handle,
				const light::network::INetEndPoint &peer);

			uint32_t install_udp_connection(uint32_t host_handle, ENetPeer *peer, uint32_t opaque);
			...
			void on_get_message_from_remote(uint32_t handle, const CommonPacket& pkt, const light::network::INetEndPoint &point);

			void handle_tcp_error(uint32_t handle, const light::utils::ErrorCode& ec);

			void handle_tcp_close(uint32_t handle);

			void async_read_tcp_connection(light::network::TcpConnection *conn, uint32_t handle);

			std::tuple<uint32_t, light::network::TcpConnection*> install_tcp_connection(int sockfd, uint32_t opaque);
