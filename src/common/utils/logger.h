...
enum { DEBUG = 1, INFO, WARNING, FATAL };
...
#define LOG_FILEANDLINE                                                        \
  __FILE__ ":" STRINGIZE(__LINE__) + light::utils::basename_index(__FILE__)

#define LOG(priority)                                                          \
  light::utils::LocalLoggerProxy(priority < light::utils::global_log_level)    \
      << "[" << light::utils::LocalLoggerProxy::get_time_str() << " "          \
      << std::this_thread::get_id() << " " << LOG_FILEANDLINE << "]["          \
      << STRINGIZE(priority) << "] "

#ifdef NDEBUG
#define DLOG(priority)                                                         \
  light::utils::LocalLoggerProxy(priority < light::utils::global_log_level)
#else
